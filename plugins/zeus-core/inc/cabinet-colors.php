...
/**
 * Expose curated cabinet-color detail pages through WordPress core XML sitemaps.
 *
 * These URLs are virtual routes rather than individual posts, so core's normal
 * post sitemap provider cannot discover them. The curated content map remains
 * the single publish flag: only mapped colors under a published collection are
 * emitted here.
 */
function zeus_get_cabinet_color_sitemap_urls() {
	$urls = array();
	$map  = zeus_get_cabinet_color_content_map();

	foreach ( $map as $style_slug => $colors ) {
		$collection = get_page_by_path( $style_slug, OBJECT, 'cabinet_collection' );
		if ( ! $collection || 'publish' !== get_post_status( $collection ) ) {
			continue;
		}

		foreach ( array_keys( $colors ) as $color_slug ) {
			$urls[] = array(
				'loc' => zeus_cabinet_color_url( $style_slug, $color_slug ),
			);
		}
	}
...
/**
 * Register a dedicated sitemap provider for virtual cabinet-color URLs.
 *
 * With pretty permalinks this appears in wp-sitemap.xml as
 * /wp-sitemap-cabinetcolors-1.xml.
 */
function zeus_register_cabinet_color_sitemap_provider() {
	...
	$provider = new class() extends WP_Sitemaps_Provider {
		public function __construct() {
			...
			$this->object_type = 'cabinetcolor';
		}

		public function get_url_list( $page_num, $object_subtype = '' ) {
			$urls         = zeus_get_cabinet_color_sitemap_urls();
			$max_urls     = wp_sitemaps_get_max_urls( $this->object_type );
			$page_num     = max( 1, (int) $page_num );
			$start_offset = ( $page_num - 1 ) * $max_urls;

			return array_slice( $urls, $start_offset, $max_urls );
		}

		public function get_max_num_pages( $object_subtype = '' ) {
			$url_count = count( zeus_get_cabinet_color_sitemap_urls() );
			if ( 0 === $url_count ) {
				return 0;
			}

			$max_urls = wp_sitemaps_get_max_urls( $this->object_type );
			return (int) ceil( $url_count / $max_urls );
		}
	};
